...
fun RecordFormScreenStandard(
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(if (state.recordId == null) "Новая запись" else "Редактирование") },
                navigationIcon = {
        ...
        Column(
            ...
            StdOperationDropdown(state, viewModel)
            ...
            StdUnitDropdown(state, viewModel)
            OutlinedTextField(
            ...
            Button(
                ...
                modifier = Modifier.fillMaxWidth().padding(vertical = NtiTokens.SpacingMd),
            ) {
                Text("Сохранить")
            }
        }
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
private fun StdOperationDropdown(state: RecordFormUiState, viewModel: RecordFormViewModel) {
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
private fun StdUnitDropdown(state: RecordFormUiState, viewModel: RecordFormViewModel) {
    var expanded by remember { mutableStateOf(false) }
    ExposedDropdownMenuBox(expanded = expanded, onExpandedChange = { expanded = it }) {
        OutlinedTextField(
